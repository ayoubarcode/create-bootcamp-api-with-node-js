...
const CourseForm = (props) => {
  ...
  const { addCourse, getSingleCourse, current, loading } = courseContext;
  ...
  const handleSubmit = (e) => {
    ...
    if (!isAllfill) {
      ...
      addCourse(bootcampId, course).then((res) => {
        if (res) {
          ...
          <div className="col-md-8 m-auto">
            <div className="card bg-white py-2 px-4">
              <div className="card-body">
                <Link
                  to={`/courses/${bootcampId}/manage`}
                  className="btn btn-link text-secondary my-3"
                >
                  <i className="fas fa-chevron-left" aria-hidden="true"></i>{' '}
                  Manage Courses
                </Link>
                <h1 className="mb-2">{bootcamp ? bootcamp.name : null}</h1>
                <h3 className="text-primary mb-4">
                  {!course_id ? 'Add Course' : 'Edit Course'}
                </h3>
